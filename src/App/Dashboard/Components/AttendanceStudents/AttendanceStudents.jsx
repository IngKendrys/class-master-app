...
export function AttendanceStudents({Option, user}) {
    ...
    const currentDate = moment(originalDate).format('YYYY-MM-DD');
    function handleVerification() {        
        const daysOfWeek = ['Domingo', 'Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado'];
        const currentDay = daysOfWeek[originalDate.getDay()];

        if (date === currentDate) {
            const schedule = data.horario;
            schedule.forEach((item) => {
                item.forEach((subItem) => {
                    if (subItem.day === currentDay) {
    ...
    useEffect(() => {
        ...
        if(date===currentDate){
            if (students) {
                students.forEach((student) => handleAttendace("asistencia", student));
            }
        } else {
            if (students) {
                students.forEach((student) => handleAttendace("", student));
            }        
        }
                        ...
                        .map(student => {
                            return (
                                <tr key={student.identification} >
                                    ...
                                    } date={date} currentDate={currentDate} schedule={correct} />
                                </tr>
                            )
                        })}
                </tbody>
